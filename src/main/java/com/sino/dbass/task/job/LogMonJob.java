...
public class LogMonJob {
    private static final Logger LOG = Logger.getLogger(LogMonJob.class);

    public static void main(String[] args) throws Exception {
        String AG_SOURCE_TAGS = ConfigurationManager.getProperty("logmon.agSourceTags");
         ...
         * ===============================================解析kafka数据================================================
         */
        SingleOutputStreamOperator kafkaSource = env.addSource(kafkaConsumer)
                .name("KafkaSource")
                .uid("KafkaSource")
         ...
         * ============================================filter过滤数据================================================
         */
        //过滤规则：1.agSourceTags为Mysql，Oracle一期，MysqlTwo为二期。2.没有配置解析规则的不做处理。3.解析规则解析出来的字段中不包含“time"的不处理
        SingleOutputStreamOperator filterSource = kafkaSource.filter(new FilterFunction<String>() {
            @Override
        ...
        SingleOutputStreamOperator readStreamEs = keyedStream.process(new LogmonToEsTransition())
                .name("ESDataHandle")
                .uid("ESDataHandle")
        ...
        //大括号是为了匿名内部类
      /*  OutputTag oracleproOutputTag = new OutputTag<String>("oracleproOutputTag", TypeInformation.of(String.class)) {
        };
        OutputTag oraclecloudOutputTag = new OutputTag<String>("oraclecloudOutputTag", TypeInformation.of(String.class)) {
        };*/
        OutputTag mysqlk8sOutputTag = new OutputTag<String>("mysqlk8sOutputTag", TypeInformation.of(String.class)) {
        };
        //侧输出流
       /* readStreamEs.getSideOutput(oracleproOutputTag)
                .addSink(new EsBultSink())
                .uid("OraclePro_ElasticsearchSink")
                .name("OraclePro_ElasticsearchSink")
                .setParallelism(sinkPrlllsm);

        readStreamEs.getSideOutput(oraclecloudOutputTag)
                .addSink(new EsBultSink())
                .uid("OracleCloud_ElasticsearchSink")
                .name("OracleCloud_ElasticsearchSink")
                .setParallelism(sinkPrlllsm)*/
        ;
